...
def read_eeg_csv(filepath):
	...
	eeg_data = df.iloc[:, 3:7].values	# Channels 3-6
...
output_filepath = os.path.abspath(os.path.join("data", "processed", f"output_{filename}"))

with open(output_filepath, mode='w', newline='') as fileX:
	writer = csv.writer(fileX)
	writer.writerows(X)

labels_filepath = os.path.abspath(os.path.join("data", "processed", f"labels_{filename}"))

with open(labels_filepath, mode='w', newline='') as fileY:
